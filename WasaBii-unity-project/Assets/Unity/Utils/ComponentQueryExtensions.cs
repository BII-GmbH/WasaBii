...
namespace CoreLibrary
{
    ...
    public static class ComponentQueryExtensions
    {
        ...
        public static bool Is<T>(this GameObject go, out T result, Search where = Search.InObjectOnly, bool includeInactive = false) where T : class 
        {
        ...
        public static bool Is<T>(this Component comp, out T result, Search where = Search.InObjectOnly, bool includeInactive = false) where T : class 
        {
            return comp.gameObject.Is<T>(out result, where, includeInactive);
        }

        /// <inheritdoc cref="Is{T}(UnityEngine.GameObject,out T,CoreLibrary.Search)" />
        public static bool Is<T>(this Collision col, out T result, Search where = Search.InObjectOnly, bool includeInactive = false) where T : class 
        {
            return col.gameObject.Is<T>(out result, where, includeInactive);
        }

        /// <param name="where">Optional search scope if the object itself does not have the component.</param>
        /// <typeparam name="T">The type of the component to find.</typeparam>
        /// <returns>true if any object in the specified search scope has a component of type T.</returns>
        public static bool Is<T>(this GameObject go, Search where = Search.InObjectOnly, bool includeInactive = false) where T : class
        {
            T unused;
            return go.Is<T>(out unused, where, includeInactive);
        }

        /// <inheritdoc cref="Is{T}(GameObject, Search)"/>
        public static bool Is<T>(this Component comp, Search where = Search.InObjectOnly, bool includeInactive = false) where T : class
        {
            return comp.gameObject.Is<T>(where, includeInactive);
        }
        
        /// <inheritdoc cref="Is{T}(GameObject, Search)"/>
        public static bool Is<T>(this Collision col, Search where = Search.InObjectOnly, bool includeInactive = false) where T : class
        {
            return col.gameObject.Is<T>(where, includeInactive);
        }
        ...
        [NotNull]
        public static T AsOrThrow<T>(this GameObject go, Search where = Search.InObjectOnly, bool includeInactive = false) where T : class
        {
        ...
        [CanBeNull]
        public static T As<T>(this GameObject go, Search where = Search.InObjectOnly, bool includeInactive = false) where T : class
        {
            switch (where)
            {
                case Search.InObjectOnly:
                ...
                case Search.InSiblings:
                    if (go.transform.parent == null) return go.As<T>(includeInactive: includeInactive);
                    foreach (var child in go.transform.parent.GetChildren())
                    {
                ...
                case Search.InWholeHierarchy:
                    if (go.transform.parent != null)
                    {
                        var parentSearch = go.transform.parent.gameObject.GetComponentInParent<T>(includeInactive);
        ...
        [CanBeNull]
        public static T As<T>(this Component comp, Search where = Search.InObjectOnly, bool includeInactive = false) where T : class
        {
            return comp.gameObject.As<T>(where, includeInactive);
        }
        
        /// <inheritdoc cref="AsOrThrow{T}(GameObject, Search)"/>
        [NotNull]
        public static T AsOrThrow<T>(this Component comp, Search where = Search.InObjectOnly, bool includeInactive = false) where T : class
        {
            return comp.gameObject.AsOrThrow<T>(where, includeInactive);
        }
        
        /// <inheritdoc cref="As{T}(GameObject, Search)"/>
        [CanBeNull]
        public static T As<T>(this Collision col, Search where = Search.InObjectOnly, bool includeInactive = false) where T : class
        {
            return col.gameObject.As<T>(where, includeInactive);
        }

        /// <param name="where">Optional search scope if the object itself does not have the component.</param>
        /// <typeparam name="T">The type of the component to find.</typeparam>
        /// <returns>A lazily generated IEnumerable of all components of type T found in the search scope. Might be empty.</returns>
        public static IEnumerable<T> All<T>(this GameObject go, Search where = Search.InObjectOnly, bool includeInactive = false) where T : class
        {
            switch (where)
            {
                case Search.InObjectOnly:
                ...
                case Search.InParents:
                    return go.GetComponentsInParent<T>(includeInactive);
                case Search.InChildren:
                    return go.GetComponentsInChildren<T>(includeInactive);
                case Search.InSiblings:
                    if (go.transform.parent == null) return go.All<T>(includeInactive: includeInactive);
                    return go.transform.parent.GetChildren().Collect(c => c.As<T>(includeInactive: includeInactive)).ToArray();
                case Search.InWholeHierarchy:
                    var parentSearch = go.transform.parent != null
                        ? go.transform.parent.gameObject.GetComponentsInParent<T>(includeInactive)
                        : new T[] { };
                    return parentSearch.AndAlso(go.GetComponentsInChildren<T>(includeInactive));
                default:
                    throw new UnsupportedSearchException(where);
            }
        }
        ...
        public static IEnumerable<T> All<T>(this Component comp, Search where = Search.InObjectOnly, bool includeInactive = false) where T : class
        {
            return comp.gameObject.All<T>(where, includeInactive);
        }
        
        /// <inheritdoc cref="All{T}(GameObject, Search)"/>
        public static IEnumerable<T> All<T>(this Collision col, Search where = Search.InObjectOnly, bool includeInactive = false) where T : class
        {
            return col.gameObject.All<T>(where, includeInactive);
        }
        ...
        /// </exception>
        public static void AssignComponent<T>(this GameObject go, out T variable, Search where = Search.InObjectOnly, bool includeInactive = false)
            where T : class
        {
        ...
        /// <param name="where">Optional search scope if the object itself does not have the component.</param>
        /// <typeparam name="T">The type of the component to find.</typeparam>
        public static void AssignComponentOrAdd<T>(this GameObject go, out T variable, Search where = Search.InObjectOnly,
        ...
        public static bool AssignIfAbsent<T>(this GameObject go, ref T variable, Search where = Search.InObjectOnly, bool includeInactive = false)
            where T : class
        {
        ...
        /// <typeparam name="T">The type of the component to find.</typeparam>
        /// <returns>true if new value was assigned, false if variable already has a value.</returns>
        public static bool AssignIfAbsentOrAdd<T>(this GameObject go, ref T variable, Search where = Search.InObjectOnly,
            bool includeInactive = false)
            where T : Component
        {
